...
int main (int argc, char* argv[]) {
    ...
    std::unique_ptr<TaskLS> task3 = std::make_unique<TaskLS_LinearMomentum>(robot_state_ptr.get(), 3, 42, rate_weight, position_weight);
    std::unique_ptr<TaskLS_StationaryEE> task4 = std::make_unique<TaskLS_StationaryEE>(robot_state_ptr.get(), 6, 42, desired_x,300,2*sqrt(150));
    std::unique_ptr<TaskLS_MoveBase> task5 = std::make_unique<TaskLS_MoveBase>(robot_state_ptr.get(), 6, 42, base_desired_x, 100, 2*sqrt(100));
    std::unique_ptr<TaskLS> task6 = std::make_unique<TaskLS_EnergyOpt>(robot_state_ptr.get(), 42);
    ...
    std::unique_ptr<TaskLS> constraints2 = std::make_unique<TaskLS_FrictionCone>(robot_state_ptr.get(), 24, 42, 0.1);
    ...
    for (i=0; i<20*hz; i++) {
        ...
        // robot->getFramePosition("FR_foot_fixed",r_c_FR);
        // robot->getFramePosition("FL_foot_fixed",r_c_FL);
        // robot->getFramePosition("RR_foot_fixed",r_c_RR);
        // robot->getFramePosition("RL_foot_fixed",r_c_RL);
        // std::cout << "FR : " << r_c_FR.e().transpose() << std::endl;
        // std::cout << "FL : " << r_c_FL.e().transpose() << std::endl;
        // std::cout << "RR : " << r_c_RR.e().transpose() << std::endl;
        // std::cout << "RL : " << r_c_RL.e().transpose() << std::endl;
